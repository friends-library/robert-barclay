...
But the Greek reads "`placing in us the word of reconciliation`" - θέμ
ενος ἐν ἡμῖν τὸν λόγον τῆς καταλλαγῆς]
...
Christ for us in His crucified body without us.
The other is the redemption wrought by Christ within us,
...
For though, to speak properly, we are not justified by them,
yet are we justified in them, and so they are necessary.
...
Now, though we judge this to be true of the best works performed by man,
...
First: that the obedience, sufferings,
...
Secondly: that it is by this inward birth of Christ in man that we are made just,
...
Thirdly: that since good works naturally flow from this birth (as heat from a fire),
...
and that man is justified in his sins.
...
Objection: Secondly, they object (fRom. 5:21 of the same chapter),
...
in multiple instances, demonstrates that we are to be made really righteous by Christ,
...
that by Christ Jesus formed in us we are justified, or truly made just.
...
any other way than in its own proper and genuine meaning of being made just.
...
for it is nothing other than a composition of the verb facio, and the adjective justus,
which is justifico, or I make just.
Likewise, the word justified is from justus and fio, meaning I become just.
So it is with other verbs of this kind,
like santifico which is from sanctus (holy) and facio, meaning I make holy.
...
then those who do know Him in them are not reprobates,
...
Argument: If no man can be justified without faith,
...
Argument: If only those who do the will of the Father can enter the kingdom of heaven;
...
but faith working through love, but the new creature, this is that which avails,
...
The former are "`works of righteousness that we have done,`" wrought in our first,
...
"`All our righteousness is as a filthy rag,`" yet this does
...
and therefore we have always denied the Catholic notion of meritum ex condigno.^
footnote:[Editor's Note:
Meritum ex condigno, is the Roman Catholic doctrine
